...
#     return df.assign(age=df['Year_Birth'].apply(lambda x:2022- x))

# #get total no of childern func.
# def get_total_children(df:pd.DataFrame) -> pd.DataFrame:
#     return df.assign(total_childern=df["Kidhome"] + df["Teenhome"])

# #get total sum of purchases func.
...
# #get the time of enrollment func.
...
# #drop the unwanted features written in the config file func.
# def drop_features(df: pd.DataFrame, keep_columns: list):
#     df = df[keep_columns]
#     return df

# #drop outlier based on threshold in config file func.
# def drop_outliers(df: pd.DataFrame, column_threshold: dict):
#     for col, threshold in column_threshold.items():
#         df = df[df[col] < threshold]
#     return df.reset_index(drop=True)

# #pipeline drop outlier and feature func. into one func.
...
#     #save the scalr as bento object
...
#     #sace pca as bento object
#     bentoml.sklearn.save("pca", pca)
#     pca_df = reduce_dimension(df, pca)

#     # Save processed data to final place
